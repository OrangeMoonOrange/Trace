...
class StreetMap:
    ...
    def load_graphdb(self, grapdb_filename):
        ...
        for id, latitude, longitude, weight in query_result:
            ...
            latitude=format(latitude,'.7f')
            longitude = format(longitude, '.7f')

            # create and store node in nodes dictionary
            self.nodes[id] = Node(float(latitude), float(longitude), id, weight)
...
#                self.segment_lookup_table[segment_edge.id] = self.segments[id]
    ...
    def _exist_node(self, node_id):

        #check node_id is in sets
        return True if self.nodes.has_key(node_id) else False
