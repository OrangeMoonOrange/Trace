...
class Graph:
    ...
    def extract(self, skeleton, density_estimate, output_filename,fclass):
        skeleton = self.identify_crossing_points(skeleton)
        main_crossings, segments = self.find_main_crossings_and_segments(skeleton)
        self.update_graph(main_crossings, segments, density_estimate, output_filename,fclass)
        # self.create_graph(main_crossings, segments, density_estimate, output_filename)
    def extract2(self, skeleton, density_estimate, output_filename,fclass):
        skeleton = self.identify_crossing_points(skeleton)
        main_crossings, segments = self.find_main_crossings_and_segments(skeleton)
        # self.update_graph(main_crossings, segments, density_estimate, output_filename)
        self.create_graph(main_crossings, segments, density_estimate, output_filename,fclass)
    def create_graph(self, main_crossings, segments, density_estimate, output_filename,fclass):
        ...
        cur.execute(
            "CREATE TABLE edges (id INTEGER, in_node INTEGER, out_node INTEGER, weight FLOAT,fclass int )")
        cur.execute("CREATE TABLE segments (id INTEGER, edge_ids TEXT,fclass int)")

        cur.execute("CREATE TABLE segments_ (id INTEGER, edge_ids TEXT,fclass int)")
        ...
        for segment in new_segments:
            ...
            for i in range(0, len(smoothed_segment) - 1):
                ...
                cur.execute("INSERT INTO edges VALUES (" + str(edge_id) + "," + str(smoothed_segment[i].id) + "," +
            ...
            for i in range(0, len(smoothed_segment) - 1):
                cur.execute("INSERT INTO edges VALUES (" + str(edge_id) + "," + str(smoothed_segment[i + 1].id) + "," +
            ...
            cur.execute("INSERT INTO segments_ VALUES (" + str(segment_id) + ",'" +
                        str(inbound_segment_edge_ids) + "'," + str(fclass) + ")")
            segment_id += 1
    ...
    # update the db file
    def update_graph(self, main_crossings, segments, density_estimate, output_filename,fclass):
...
if __name__ == '__main__':
    ...
    g.extract2(skeleton.astype(np.bool).astype(np.int), skeleton, output_filename,1)
