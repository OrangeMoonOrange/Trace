...
class Graph:
    ...
    def extract(self, skeleton, density_estimate, fclass):
        skeleton = self.identify_crossing_points(skeleton)
        main_crossings, segments = self.find_main_crossings_and_segments(skeleton)
        self.update_graph(main_crossings, segments, density_estimate, fclass)
        # self.create_graph(main_crossings, segments, density_estimate, output_filename)

    def extract2(self, skeleton, density_estimate,fclass,pointtable,linetable):
        skeleton = self.identify_crossing_points(skeleton)
        main_crossings, segments = self.find_main_crossings_and_segments(skeleton)
        # self.update_graph(main_crossings, segments, density_estimate, output_filename)
        self.create_graph(main_crossings, segments, density_estimate, fclass,pointtable,linetable)

    def create_graph(self, main_crossings, segments, density_estimate, fclass,pointtable,linetable):
        nodes, new_segments, intersections = self.create_nodes_and_new_segments(main_crossings, segments,
                                                                          density_estimate)
        # 获取数据库资源
        cur = db.cur    # 数据库连接的游标
        con = db.conn   # 数据库连接
        nodes_table_name=pointtable  # 全量更新的表名
        edges_table_name=linetable   #

        cur.execute("CREATE TABLE "+str(nodes_table_name)+" (id INTEGER, latitude double, longitude double, weight FLOAT)")
        cur.execute("CREATE TABLE IF NOT EXISTS "+str(edges_table_name)+" (id INTEGER, in_node INTEGER, out_node INTEGER, weight FLOAT,fclass int )")
        # cur.execute("CREATE TABLE IF NOT EXISTS segments (id INTEGER, edge_ids TEXT,fclass int)")
        # cur.execute("CREATE TABLE IF NOT EXISTS intersections (node_id INTEGER)")
        con.commit()
        ...
        for segment in new_segments:
            ...
            for node in smoothed_segment:
                if (node.id is None):
                    node.id = node_id
                    cur.execute("INSERT INTO "+str(nodes_table_name)+" (id , latitude , longitude, weight ) VALUES (" + str(node.id) + "," + str(node.latitude) + "," + str(
                        node.longitude) + "," + str(node.weight) + ")")
                    node_id += 1

            outbound_segment_edge_ids = []
            for i in range(0, len(smoothed_segment) - 1):
                cur.execute("INSERT INTO "+str(edges_table_name)+" (id , in_node , out_node , weight ,fclass) VALUES (" + str(edge_id) + "," + str(smoothed_segment[i].id) + "," +
                            str(smoothed_segment[i + 1].id) + "," + str(segment_weight) + "," + str(fclass) + ")")
            ...
            for i in range(0, len(smoothed_segment) - 1):
                cur.execute("INSERT INTO "+str(edges_table_name)+" (id , in_node , out_node , weight ,fclass) VALUES (" + str(edge_id) + "," + str(smoothed_segment[i + 1].id) + "," +
                            str(smoothed_segment[i].id) + "," + str(segment_weight) + "," + str(fclass) + ")")
        ...
        #     #     node.longitude) + "," + str(node.weight) + ")")
        #     cur.execute("INSERT INTO segments (id, edge_ids,fclass) VALUES (" + str(segment_id) + ",'" +
        #                 str(outbound_segment_edge_ids) + "'," + str(fclass) + ")")
        #     segment_id += 1
        #
        #     cur.execute("INSERT INTO segments (id, edge_ids,fclass) VALUES (" + str(segment_id) + ",'" +
        #                 str(inbound_segment_edge_ids) + "'," + str(fclass) + ")")
        #     segment_id += 1
        #
        # for intersection in intersections:
        #     cur.execute("INSERT INTO intersections (node_id ) VALUES (" + str(intersection.id) + ")")

        con.commit()
        con.close()

    # update the db file
    def update_graph(self, main_crossings, segments, density_estimate, fclass):
        nodes, new_segments, intersections = self.create_nodes_and_new_segments(main_crossings, segments,
                                                                                density_estimate)
        # 获取数据库资源
        cur = db.cur
        ...
        cursor = db.ExecQuery("SELECT max(id) from "+str(base_pointtable)+"")
        for x in cursor:
            node_id = x[0] + 1

        curd=db.ExecQuery("SELECT max(id) from "+str(base_linetable)+"")
        for x in curd:
            edge_id = x[0] + 1
        ...
        for segment in new_segments:
            ...
            for node in smoothed_segment:
                if (1 == 1):
                    node.id = node_id
                    cur.execute("INSERT INTO "+str(base_pointtable)+" VALUES (" + str(node.id) + "," + str(node.latitude) + "," + str(
                        node.longitude) + "," + str(node.weight) + ")")
                    node_id += 1

            outbound_segment_edge_ids = []
            for i in range(0, len(smoothed_segment) - 1):
                cur.execute("INSERT INTO "+str(base_linetable)+" VALUES (" + str(edge_id) + "," + str(smoothed_segment[i].id) + "," +
                            str(smoothed_segment[i + 1].id) + "," + str(segment_weight) + "," + str(fclass) + ")")
                outbound_segment_edge_ids.append(edge_id)
                edge_id += 1
            ...
            for i in range(0, len(smoothed_segment) - 1):
                cur.execute(
                    "INSERT INTO "+str(base_linetable)+" VALUES (" + str(edge_id) + "," + str(smoothed_segment[i + 1].id) + "," + str(
                        smoothed_segment[i].id) + "," + str(segment_weight) + "," + str(fclass) + ")")
            ...
            # cur.execute(
            #     "INSERT INTO segments VALUES (" + str(segment_id) + ",'" + str(outbound_segment_edge_ids) + "',1)")
            # segment_id += 1
            #
            # cur.execute("INSERT INTO segments VALUES (" + str(segment_id) + ",'" + str(inbound_segment_edge_ids) + "',1)")
            # segment_id += 1
        #     cur.execute("INSERT INTO segments VALUES (" + str(segment_id) + ",'" +
        #                 str(outbound_segment_edge_ids) + "'," + str(fclass) + ")")
        #     segment_id += 1
        #
        #     cur.execute("INSERT INTO segments VALUES (" + str(segment_id) + ",'" +
        ...
        #
        # for intersection in intersections:
        #     cur.execute("INSERT INTO intersections VALUES (" + str(intersection_id) + ")")
        #     intersection_id += 1
        conn.commit()
        conn.close()
...
if __name__ == '__main__':
    ...
    output_filename = "skeleton_map_1m.db"
    ...
    sql = SQLHelper("192.168.253.100", "root", "123", "kswq")

    # g.extract2(skeleton_g.astype(np.bool).astype(np.int), skeleton_g, fclass, db, pointtable, linetable)
    g.extract2(skeleton.astype(np.bool).astype(np.int), skeleton, 1,sql,"node","line")
