...
class ProcessMapMatches:
    ...
    def process_all_matched_trips(self, graphdb_filename, matched_trips_directory, output_db_filename):
        ...
        for i in range(0, len(all_matched_trip_files)):
            ...
            for record in matched_trip_records:
                if (len(record) < 7):
                    ...
                    no_obs_time_ranges.append((float(obs_time)-time , float(obs_time)+time))
                else:

                    obs_lat, obs_lon, obs_time, state_in_node_lat, state_in_node_lon, state_out_node_lat, state_out_node_lon = record

                    key = ((float(state_in_node_lat), float(state_in_node_lon)),
                           (float(state_out_node_lat), float(state_out_node_lon)))
                    # print key
                    curr_state_edge = self.graphdb.edge_coords_lookup_table[key]
                    #
                    # print "============================="
                    # print type(curr_state_edge)
                    # print "============================="
        ...
        for segment_id in all_segment_obs:
            ...
            if (num_segment_traces > trace_num):
...
if __name__ == '__main__':
    ...
    matched_trips_directory = "trips/matched_trips_1m/"
    output_filename = "skeleton_maps/skeleton_map_1m_mm1.db"

    # graphdb_filename = "skeleton_maps/skeleton_map_1m_mm1_tr.db"
    # matched_trips_directory = "trips/matched_trips_1m_mm1_tr/"
    # output_filename = "skeleton_maps/skeleton_map_1m_mm2.db"
