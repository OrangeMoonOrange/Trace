...
class run:
    def __init__(self, configfile="../docs/default.ini"):
        ...
        self.trace = db.SQLServer(server=cf.get('sourcesdb','dbip'), user=cf.get('sourcesdb','USRID'), password=cf.get('sourcesdb','PSW'), database=cf.get('sourcesdb','DBNAME'))
    ...
    def fromFullData2Map(self,start,end):
        ...
        trips = Trip_get(start, end, self.trace).load_trip_from_db()
